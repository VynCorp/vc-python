...
and an async bulk export job with polling. Exports require the Enterprise tier
and are skipped with a note on lower tiers.
...
def main() -> None:
    ...
    # --- Find some companies to export (Free tier) ---
    print("=== Data Export ===\n")
    uids = []
    ...
    # --- Quick export of specific companies (Enterprise) ---
    ...
    # --- Bulk export with filters (Enterprise) ---
    with section("2. Bulk Export (filtered by canton)"):
        job = client.exports.create(format="csv", canton="ZH", status="active", max_rows=100)
    ...
    # --- Company statistics (Free tier) ---
    with section("3. Database Statistics"):
        s = client.companies.statistics().data
